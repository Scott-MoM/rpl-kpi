...
def get_admin_overview() -> AdminOverview:
    return get_admin_service().get_overview()
...
def list_password_reset_requests() -> list[PendingPasswordReset]:
    return get_admin_service().list_pending_password_resets()
...
def list_audit_logs(
    ...
    return get_admin_service().list_audit_logs(search=search, action=action, limit=limit)
...
async def import_beacon_csv(
    ...
    return await get_admin_service().import_beacon_csvs(
...
def start_manual_sync(payload: StartSyncRequest) -> SyncJobState:
    return get_sync_service().start_manual_sync(payload)
...
def get_latest_manual_sync(user_email: str | None = None) -> SyncJobState | None:
    return get_sync_service().get_latest_job(user_email=user_email)


@router.get("/sync/{job_id}", response_model=SyncJobState)
def get_manual_sync(job_id: str) -> SyncJobState:
    return get_sync_service().get_job(job_id)
...
def stop_manual_sync(job_id: str, payload: StartSyncRequest) -> SyncJobState:
    return get_sync_service().stop_job(job_id, user_email=payload.user_email, region=payload.region)
...
def clear_manual_sync(job_id: str, payload: StartSyncRequest) -> SyncJobState:
    return get_sync_service().clear_job(job_id, user_email=payload.user_email, region=payload.region)
